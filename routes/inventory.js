...
const client = new MongoClient('mongodb://127.0.0.1:27017/pdbms', { useNewUrlParser: true, useUnifiedTopology: true });


   const databaseName = 'pdbms';
...
router.get('/edit/:id', async (req, res) => {
  ...
  try {
    ...
    res.render('inventory/inventory_edit', { db_rows_array: result });
  } catch (error) {
...
router.post('/edit/:id', async (req, res) => {
  ...
  try {
    ...
    res.redirect('/inventory/display');
  } catch (error) {
    console.error('Error:', error);
    res.status(500).send('Internal Server Error');
  } finally {
...
router.get('/display', async (req, res) => {
  try {
    ...
    res.render('inventory/inventory_display', { db_rows_array: documents});
...
router.get('/show/:id', async (req, res) => {
  ...
  try {
    ...
    // Render an HTML page with the data for editing
    res.render('inventory/inventory_show', { db_rows_array: result });
  } catch (error) {
    console.error('Error:', error);
    res.status(500).send('Internal Server Error');
  } finally {
